...
those 33; it must never imply broader coverage exists. It is content and
documentation, not a component implementation — it ships no headless
components, no example app, no helper packages of its own.
...
- `SKILL.md` — the skill: the catalog's partial scope and exact component
  list (33 of 491, spanning buttons/links, forms, overlays, media/data,
  content, and the breadcrumb navigation family), the two architecture
  decisions made before any component was written (autonomous custom
  elements over customized built-ins, light-DOM-only), the two structural
  patterns components use (wrap a real native element; self-is-the-wrapper)
  plus the one-off "upgrade in place" pilot on breadcrumb, what's
  deliberately excluded and why (every `*ListItem`/table-sub-element family
  other than breadcrumb; the 92 national personal identifier components),
  the custom-element consumption idiom, class-hook theming, and pointers
  into the catalog-wide naming reference rather than a restatement of it.
...
- **Claiming or implying full 491/491 catalog parity.** The 33-component
  scope, and the 458 components this catalog does not ship, are stated
  plainly wherever this skill discusses coverage — never softened or
  omitted.
...
- **Autonomous custom elements over customized built-in elements.**
  ...
  node wrapping the real semantic element in most of the 33 components.
...
- [x] `SKILL.md` names what's deliberately excluded (the `*ListItem`/
      table-sub-element families other than breadcrumb; the 92 national
      personal identifier components) and why.
